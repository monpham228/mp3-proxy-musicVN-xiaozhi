"""
MCP stdio <-> WebSocket pipe for Xiaozhi Music Server
Based on mcp-calculator pipe implementation

Usage:
    export MCP_ENDPOINT=wss://api.xiaozhi.me/mcp/?token=YOUR_TOKEN
    python mcp_pipe.py

Or run specific server:
...
def build_server_command(target=None):
    """Build command and env for the server process"""
    if target is None:
